...
const useStorage = () => {
   ...
   */
  const getFileUrl = useCallback((bucketId, fileId, width = 0, height = 0) => {
    try {
      if (width && height) {
        return {
          success: true,
   ...
   */
  const getProductImageUrl = useCallback((fileId, width = 500, height = 500) => {
   ...
   * Get blog image URL
   */
  const getBlogImageUrl = useCallback((fileId, width = 800, height = 450) => {
   ...
   * Get user avatar URL
   */
  const getUserAvatarUrl = useCallback((fileId, width = 100, height = 100) => {
   ...
   */
  const uploadFile = useCallback(async (bucketId, file) => {
    try {
   ...
   * Upload blog image
   */
  const uploadBlogImage = useCallback(async (file) => {
   ...
   */
  const deleteFile = useCallback(async (bucketId, fileId) => {
    try {
   ...
   */
  const listFiles = useCallback(async (bucketId) => {
    try {
