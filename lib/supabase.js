...
/**
 * Drop a stored session whose token was minted ahead of real time.
 *
 * If the machine clock runs fast (or is corrected backwards afterwards), the
 * saved access token carries an `iat` in the future and every PostgREST call
 * fails with "JWT issued at future" until the session is cleared by hand.
 * Checking it once at startup turns that dead end into a normal logged-out
 * state. Returns true when a session was discarded.
 */
export async function clearSkewedSession() {
  ...
  try {
    ...
    // 120s of slack: small drift is normal and the server tolerates it.
    const skewed = claims.iat && claims.iat * 1000 > Date.now() + 120000;
    if (!skewed) return false;

    console.warn(
      "Discarding a session token issued in the future (check the system clock).",
    );
    await supabase.auth.signOut();
    return true;
  } catch (error) {
    console.error("Could not inspect the stored session:", error);
    return false;
  }
}
...
// Read-only view over `user` without email, phone or saved addresses. Use it
// for anything about *another* person; the table itself is now owner-only.
export const PUBLIC_PROFILES_COLLECTION_ID = "public_profiles";
...
export const COLLECTIONS = {
  ...
  PUBLIC_PROFILES: PUBLIC_PROFILES_COLLECTION_ID,
...
// ============================================
// Postgres rows carry `id` / `created_at` / `updated_at`, while the call sites
// inherited from Appwrite read `$id` / `$createdAt` / `$updatedAt`. Expose both
// without touching the row's own shape: the aliases are non-enumerable, so they
// never leak into a spread that later becomes an INSERT or UPDATE payload.
function withCompatIds(row) {
  if (!row || typeof row !== "object" || "$id" in row) return row;
  Object.defineProperties(row, {
    $id: { get: () => row.id, enumerable: false, configurable: true },
    $createdAt: { get: () => row.created_at, enumerable: false, configurable: true },
    $updatedAt: { get: () => row.updated_at, enumerable: false, configurable: true },
  });
  return row;
}
...
async function listDocuments(_dbId, table, queries = []) {
  ...
  if (limit != null) {
    ...
    // Only use a Range header when paginating past the start. `.range()` on a
    // table with fewer rows than `start` returns PostgREST 416, so for the
    // common offset-0 case use `.limit()` (a query param) which never 416s.
    if (start > 0) {
  ...
  if (error) {
    // 416 / PGRST103 = requested range is past the end of the table (e.g. an
    // empty table or paging beyond the last row). Treat as an empty page.
    if (error.code === "PGRST103") {
      return { documents: [], total: count ?? 0 };
    }
    throw wrapError(error);
  }
  const documents = (data || []).map(withCompatIds);
  return { documents, total: count ?? (data ? data.length : 0) };
}
